...
const getAllTotal = catchAsync(async (req, res, next) => {
...
const outletsStatsInfo = catchAsync(async (req, res, next) => {
  ...
  for (let i in visitor) {
    if (!(visitor[i].outlet.name in outletVisitors)) {
      ...
      outletVisitors[visitor[i].outlet.name] =
        outletVisitors[visitor[i].outlet.name] + 1;
    }
  }
  ...
  for (let i in transaction) {
    if (!(transaction[i].outlet.name in outletTransactions)) {
      ...
      outletTransactions[transaction[i].outlet.name] =
        outletTransactions[transaction[i].outlet.name] + 1;
    }
  }
  ...
  for (let i in transaction) {
    if (!(transaction[i].outlet.name in outletTotalEarnings)) {
      outletTotalEarnings[transaction[i].outlet.name] =
        transaction[i].totalPrice;
    } else {
      outletTotalEarnings[transaction[i].outlet.name] =
        outletTotalEarnings[transaction[i].outlet.name] +
        transaction[i].totalPrice;
...
const getIndividualCustomerStats = catchAsync(async (req, res, next) => {
...
const getOverAllCustomerStats = catchAsync(async (req, res, next) => {
  ...
  for (let i in user) {
    if (user[i].nationality in listNationality) {
      console.log("if", user[i].nationality);
      listNationality[user[i].nationality] =
        listNationality[user[i].nationality] + 1;

      nationalityArr.forEach((item) => {
        if (item[0] === user[i].nationality) {
          item[1] = item[1] + 1;
        }
      });
    } else {
      listNationality[user[i].nationality] = 1;
      nationalityArr.push([user[i].nationality, 1]);
    }
  }
...
const getIndividualOutletStats = catchAsync(async (req, res, next) => {
  ...
  const total_outlet_main_visit = await Visitor.count({
    outlet: outlet_id,
    isBranch: false,
  });

  const list_outlet_main_visit = await Visitor.find({
    outlet: outlet_id,
    isBranch: false,
  });
  const list_outlet_main_transaction = await Transaction.find({
    outlet: outlet_id,
    isBranch: false,
  });

  const transaction_branch = await Transaction.find({
    outlet: outlet_id,
    isBranch: true,
  });
...
const getBranchOutletStats = catchAsync(async (req, res, next) => {
  const branch_id = req.params.id;

  const count_branch_transaction = await Transaction.count({
    branch: branch_id,
    isBranch: true,
  });
  const count_branch_visit = await Visitor.count({
    branch: branch_id,
    isBranch: true,
  });

  const branch_transaction = await Transaction.find({
    branch: branch_id,
    isBranch: true,
  });
  const branch_visit = await Visitor.find({
    branch: branch_id,
    isBranch: true,
  });
...
const getOverallOutletStats = catchAsync(async (req, res, next) => {
...
const outletStatsCustomer = catchAsync(async (req, res, next) => {
...
const totalOutletLiveTracker = catchAsync(async (req, res, next) => {
...
const totalBranchLiveTracker = catchAsync(async (req, res, next) => {
...
const getpecificMonthYearVisitorOutlet = catchAsync(async (req, res, next) => {
  const { outlet, month, day, year } = req.body;
  const visit = await Visitor.find({
    outlet: outlet,
    month: month,
    year: year,
...
const getSpecificDateVisitorOutlet = catchAsync(async (req, res, next) => {
  const { outlet, month, day, year } = req.body;
...
const getpecificMonthYearTransactionOutlet = catchAsync(
  async (req, res, next) => {
    const { outlet, month, year } = req.body;
    const transaction = await Transaction.find({
      outlet: outlet,
      month: month,
      year: year,
...
const getSpecificDateTransactionOutlet = catchAsync(async (req, res, next) => {
  const { outlet, month, day, year } = req.body;
...
const getpecificMonthYearUserOutlet = catchAsync(async (req, res, next) => {
  const { outlet, month, year } = req.body;
  const user = await User.find({ outlet: outlet, month: month, year: year });
...
const getSpecificDateUserOutlet = catchAsync(async (req, res, next) => {
  const { outlet, month, day, year } = req.body;
...
const getpecificMonthYearRegisterTransactionOutlet = catchAsync(
  async (req, res, next) => {
    const { outlet, month, year } = req.body;
    const registerTransaction = await RegisterTransaction.find({
      outlet: outlet,
...
const getSpecificDateRegisterTransactionOutlet = catchAsync(
  async (req, res, next) => {
    const { outlet, month, day, year } = req.body;
...
const searchOutletFilter = async (req, res) => {
  try {
    ...
    const outlet = await Outlet.find(searchObj).select("-password");
...
module.exports = {
  ...
  getCategories,
  outletsStatsInfo,
  getIndividualCustomerStats,
  getOverAllCustomerStats,
  getIndividualOutletStats,
  getBranchOutletStats,
  getOverallOutletStats,
  outletStatsCustomer,
  totalOutletLiveTracker,
  totalBranchLiveTracker,
  searchOutletFilter,

  getpecificMonthYearVisitorOutlet,
  getSpecificDateVisitorOutlet,
  getpecificMonthYearTransactionOutlet,
  getSpecificDateTransactionOutlet,
  getpecificMonthYearUserOutlet,
  getSpecificDateUserOutlet,
  getpecificMonthYearRegisterTransactionOutlet,
  getSpecificDateRegisterTransactionOutlet,
};
